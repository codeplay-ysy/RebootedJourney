...
- 窗口函数在当前表达式后面接 OVER (PARTITION BY xxx ORDER BY xxx)，PARTITION BY 用于分组。比如说按照名字分组，这样计算sum的时候就会计算每个组的sum。而ORDER BY 会给定顺序。这个可以参考最下面的那个图，也可能对结果有影响。
rank()用于排级别。
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTk5MDUzODk0NywtNTAzMjIzMzY1LDEyMj
Q0NTYzNTUsOTE4NTU3MzgxLDY4MzI0MDQwMCwtMjkwNTE3Mzc2
LDE2OTE5Mzk4MywtMTgxOTgxODYzMCwtODk5ODgwMTU1LC00Nj
YxMDE3MzEsLTIwOTE5NDEwMDIsMTczMTA1MzAwOSwxMDU1NzI0
MTg5LC0xMDcwOTIyODkwLDQ1NDgzNTkxMywxNDk4MDYxOTk4LC
0xNjkxOTcyNzYyLDE4NDIzNjQ0ODgsLTEzMjk5OTIxNTYsLTE0
Mjg2NDMwMDhdfQ==
-->